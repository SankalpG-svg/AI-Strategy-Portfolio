...
print("Waking up the Master Brain...")
if os.path.exists("custom_connect4_cnn.zip"):
    model = PPO.load("custom_connect4_cnn", env=env, tensorboard_log="./tensorboard_logs/")
else:
    print("Creating new PPO model for self-play...")
    model = PPO("CnnPolicy", env, policy_kwargs=policy_kwargs, verbose=1, tensorboard_log="./tensorboard_logs/")

# Pass the model to the environment for self-play!
env.rl_model = model
